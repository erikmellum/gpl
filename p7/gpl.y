        ...
        if(!symbol_table->lookup(id) && !symbol_table->lookup(id+"[0]"))
        {
            ...
            else if(gpl_type == ANIMATION_BLOCK)
            {
               //cout << "Current Object: " << cur_object_under_construction->type() << endl << "Param: ";
                ...
                if(cur_object_under_construction->type() != $3->eval_animation_block()->get_parameter_symbol()->getGameObject()->type())
                {
                ...
                else
                {
                    ...
                    if(status == MEMBER_NOT_OF_GIVEN_TYPE)
                        Error::error(Error::TYPE_MISMATCH_BETWEEN_ANIMATION_BLOCK_AND_OBJECT, *$1, cur_object_name);
                }
            }
